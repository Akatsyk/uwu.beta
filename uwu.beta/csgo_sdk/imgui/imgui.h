...
// Obsolete functions
// Helpers (ImGuiOnceUponAFrame, ImGuiTextFilter, ImGuiTextBuffer, ImGuiStorage, ImGuiListClipper, Imcolor_t)
...
struct Imcolor_t;                     // Helper functions to create a color that can be converted to either u32 or float4 (*OBSOLETE* please avoid using)
...
typedef int ImGuicolor_tEditFlags;    // -> enum ImGuicolor_tEditFlags_  // Flags: for color_tEdit4(), color_tPicker4() etc.
...
namespace ImGui
{
    ...
    // Styles
    IMGUI_API void          Stylecolor_tsDark(ImGuiStyle* dst = NULL);    // new, recommended style (default)
    IMGUI_API void          Stylecolor_tsClassic(ImGuiStyle* dst = NULL); // classic imgui style
    IMGUI_API void          Stylecolor_tsLight(ImGuiStyle* dst = NULL);   // best used with borders and a custom, thicker font
    ...
    IMGUI_API void          PushStylecolor_t(ImGuiCol idx, ImU32 col);
    IMGUI_API void          PushStylecolor_t(ImGuiCol idx, const ImVec4& col);
    IMGUI_API void          PopStylecolor_t(int count = 1);
    IMGUI_API void          PushStyleVar(ImGuiStyleVar idx, float val);
    IMGUI_API void          PushStyleVar(ImGuiStyleVar idx, const ImVec2& val);
    IMGUI_API void          PopStyleVar(int count = 1);
    IMGUI_API const ImVec4& GetStylecolor_tVec4(ImGuiCol idx);                                // retrieve style color as stored in ImGuiStyle structure. use to feed back into PushStylecolor_t(), otherwise use Getcolor_tU32() to get style color with style alpha baked in.
    IMGUI_API ImFont*       GetFont();                                                      // get current font
    IMGUI_API float         GetFontSize();                                                  // get current font size (= height in pixels) of current font with current scale applied
    IMGUI_API ImVec2        GetFontTexUvWhitePixel();                                       // get UV coordinate for a while pixel, useful to draw custom shapes via the ImDrawList API
    IMGUI_API ImU32         Getcolor_tU32(ImGuiCol idx, float alpha_mul = 1.0f);              // retrieve given style color with style alpha applied and optional extra alpha multiplier
    IMGUI_API ImU32         Getcolor_tU32(const ImVec4& col);                                 // retrieve given color with style alpha applied
    IMGUI_API ImU32         Getcolor_tU32(ImU32 col);                                         // retrieve given color with style alpha applied
    ...
    IMGUI_API void          Textcolor_ted(const ImVec4& col, const char* fmt, ...)            IM_FMTARGS(2); // shortcut for PushStylecolor_t(ImGuiCol_Text, col); Text(fmt, ...); PopStylecolor_t();
    IMGUI_API void          Textcolor_tedV(const ImVec4& col, const char* fmt, va_list args)  IM_FMTLIST(2);
    IMGUI_API void          TextDisabled(const char* fmt, ...)                              IM_FMTARGS(1); // shortcut for PushStylecolor_t(ImGuiCol_Text, style.color_ts[ImGuiCol_TextDisabled]); Text(fmt, ...); PopStylecolor_t();
    ...
    // Widgets: color_t Editor/Picker (tip: the color_tEdit* functions have a little colored preview square that can be left-clicked to open a picker, and right-clicked to open an option menu.)
    // - Note that in C++ a 'float v[X]' function argument is the _same_ as 'float* v', the array syntax is just a way to document the number of elements that are expected to be accessible.
    // - You can pass the address of a first float element out of a contiguous structure, e.g. &myvector.x
    IMGUI_API bool          color_tEdit3(const char* label, float col[3], ImGuicolor_tEditFlags flags = 0);
    IMGUI_API bool          color_tEdit4(const char* label, float col[4], ImGuicolor_tEditFlags flags = 0);
    IMGUI_API bool          color_tPicker3(const char* label, float col[3], ImGuicolor_tEditFlags flags = 0);
    IMGUI_API bool          color_tPicker4(const char* label, float col[4], ImGuicolor_tEditFlags flags = 0, const float* ref_col = NULL);
    IMGUI_API bool          color_tButton(const char* desc_id, const ImVec4& col, ImGuicolor_tEditFlags flags = 0, ImVec2 size = ImVec2(0,0));  // display a colored square/button, hover for details, return true when pressed.
    IMGUI_API void          Setcolor_tEditOptions(ImGuicolor_tEditFlags flags);                     // initialize current options (generally on application startup) if you want to select a default format, picker type, etc. User will be able to change many settings, unless you pass the _NoOptions flag to your calls.
    ...
    IMGUI_API const char*   GetStylecolor_tName(ImGuiCol idx);                                    // get a string corresponding to the enum value (for display, saving, etc.).
    ...
    // color_t Utilities
    IMGUI_API ImVec4        color_tConvertU32ToFloat4(ImU32 in);
    IMGUI_API ImU32         color_tConvertFloat4ToU32(const ImVec4& in);
    IMGUI_API void          color_tConvertRGBtoHSV(float r, float g, float b, float& out_h, float& out_s, float& out_v);
    IMGUI_API void          color_tConvertHSVtoRGB(float h, float s, float v, float& out_r, float& out_g, float& out_b);
...
// Enumeration for PushStylecolor_t() / PopStylecolor_t()
enum ImGuiCol_
{
    ImGuiCol_Text,
...
// Flags for color_tEdit3() / color_tEdit4() / color_tPicker3() / color_tPicker4() / color_tButton()
enum ImGuicolor_tEditFlags_
{
    ImGuicolor_tEditFlags_None            = 0,
    ImGuicolor_tEditFlags_NoAlpha         = 1 << 1,   //              // color_tEdit, color_tPicker, color_tButton: ignore Alpha component (will only read 3 components from the input pointer).
    ImGuicolor_tEditFlags_NoPicker        = 1 << 2,   //              // color_tEdit: disable picker when clicking on colored square.
    ImGuicolor_tEditFlags_NoOptions       = 1 << 3,   //              // color_tEdit: disable toggling options menu when right-clicking on inputs/small preview.
    ImGuicolor_tEditFlags_NoSmallPreview  = 1 << 4,   //              // color_tEdit, color_tPicker: disable colored square preview next to the inputs. (e.g. to show only the inputs)
    ImGuicolor_tEditFlags_NoInputs        = 1 << 5,   //              // color_tEdit, color_tPicker: disable inputs sliders/text widgets (e.g. to show only the small preview colored square).
    ImGuicolor_tEditFlags_NoTooltip       = 1 << 6,   //              // color_tEdit, color_tPicker, color_tButton: disable tooltip when hovering the preview.
    ImGuicolor_tEditFlags_NoLabel         = 1 << 7,   //              // color_tEdit, color_tPicker: disable display of inline text label (the label is still forwarded to the tooltip and picker).
    ImGuicolor_tEditFlags_NoSidePreview   = 1 << 8,   //              // color_tPicker: disable bigger color preview on right side of the picker, use small colored square preview instead.
    ImGuicolor_tEditFlags_NoDragDrop      = 1 << 9,   //              // color_tEdit: disable drag and drop target. color_tButton: disable drag and drop source.

    // User Options (right-click on widget to change some of them).
    ImGuicolor_tEditFlags_AlphaBar        = 1 << 16,  //              // color_tEdit, color_tPicker: show vertical alpha bar/gradient in picker.
    ImGuicolor_tEditFlags_AlphaPreview    = 1 << 17,  //              // color_tEdit, color_tPicker, color_tButton: display preview as a transparent color over a checkerboard, instead of opaque.
    ImGuicolor_tEditFlags_AlphaPreviewHalf= 1 << 18,  //              // color_tEdit, color_tPicker, color_tButton: display half opaque / half checkerboard, instead of opaque.
    ImGuicolor_tEditFlags_HDR             = 1 << 19,  //              // (WIP) color_tEdit: Currently only disable 0.0f..1.0f limits in RGBA edition (note: you probably want to use ImGuicolor_tEditFlags_Float flag as well).
    ImGuicolor_tEditFlags_DisplayRGB      = 1 << 20,  // [Display]    // color_tEdit: override _display_ type among RGB/HSV/Hex. color_tPicker: select any combination using one or more of RGB/HSV/Hex.
    ImGuicolor_tEditFlags_DisplayHSV      = 1 << 21,  // [Display]    // "
    ImGuicolor_tEditFlags_DisplayHex      = 1 << 22,  // [Display]    // "
    ImGuicolor_tEditFlags_Uint8           = 1 << 23,  // [DataType]   // color_tEdit, color_tPicker, color_tButton: _display_ values formatted as 0..255.
    ImGuicolor_tEditFlags_Float           = 1 << 24,  // [DataType]   // color_tEdit, color_tPicker, color_tButton: _display_ values formatted as 0.0f..1.0f floats instead of 0..255 integers. No round-trip of value via integers.
    ImGuicolor_tEditFlags_PickerHueBar    = 1 << 25,  // [Picker]     // color_tPicker: bar for Hue, rectangle for Sat/Value.
    ImGuicolor_tEditFlags_PickerHueWheel  = 1 << 26,  // [Picker]     // color_tPicker: wheel for Hue, triangle for Sat/Value.
    ImGuicolor_tEditFlags_InputRGB        = 1 << 27,  // [Input]      // color_tEdit, color_tPicker: input and output data in RGB format.
    ImGuicolor_tEditFlags_InputHSV        = 1 << 28,  // [Input]      // color_tEdit, color_tPicker: input and output data in HSV format.

    // Defaults Options. You can set application defaults using Setcolor_tEditOptions(). The intent is that you probably don't want to
    // override them in most of your calls. Let the user choose via the option menu and/or call Setcolor_tEditOptions() once during startup.
    ImGuicolor_tEditFlags__OptionsDefault = ImGuicolor_tEditFlags_Uint8|ImGuicolor_tEditFlags_DisplayRGB|ImGuicolor_tEditFlags_InputRGB|ImGuicolor_tEditFlags_PickerHueBar,

    // [Internal] Masks
    ImGuicolor_tEditFlags__DisplayMask    = ImGuicolor_tEditFlags_DisplayRGB|ImGuicolor_tEditFlags_DisplayHSV|ImGuicolor_tEditFlags_DisplayHex,
    ImGuicolor_tEditFlags__DataTypeMask   = ImGuicolor_tEditFlags_Uint8|ImGuicolor_tEditFlags_Float,
    ImGuicolor_tEditFlags__PickerMask     = ImGuicolor_tEditFlags_PickerHueWheel|ImGuicolor_tEditFlags_PickerHueBar,
    ImGuicolor_tEditFlags__InputMask      = ImGuicolor_tEditFlags_InputRGB|ImGuicolor_tEditFlags_InputHSV

    // Obsolete names (will be removed)
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    , ImGuicolor_tEditFlags_RGB = ImGuicolor_tEditFlags_DisplayRGB, ImGuicolor_tEditFlags_HSV = ImGuicolor_tEditFlags_DisplayHSV, ImGuicolor_tEditFlags_HEX = ImGuicolor_tEditFlags_DisplayHex  // [renamed in 1.69]
#endif
};
...
// and ImGui::PushStylecolor_t(ImGuiCol_XXX)/PopStylecolor_t() for colors.
//-----------------------------------------------------------------------------
...
struct ImGuiStyle
{
    ...
    ImGuiDir    color_tButtonPosition;        // Side of the color button in the color_tEdit4 widget (left/right). Defaults to ImGuiDir_Right.
    ...
    ImVec4      color_ts[ImGuiCol_COUNT];
...
// Helper: Imcolor_t() implicitly converts colors to either ImU32 (packed 4x1 byte) or ImVec4 (4x1 float)
// Prefer using IM_COL32() macros if you want a guaranteed compile-time ImU32 for usage with ImDrawList API.
// **Avoid storing Imcolor_t! Store either u32 of ImVec4. This is not a full-featured color class. MAY OBSOLETE.
// **None of the ImGui API are using Imcolor_t directly but you can use it as a convenience to pass colors in either ImU32 or ImVec4 formats. Explicitly cast to ImU32 or ImVec4 if needed.
struct Imcolor_t
{
    ImVec4              Value;

    Imcolor_t()                                                       { Value.x = Value.y = Value.z = Value.w = 0.0f; }
    Imcolor_t(int r, int g, int b, int a = 255)                       { float sc = 1.0f/255.0f; Value.x = (float)r * sc; Value.y = (float)g * sc; Value.z = (float)b * sc; Value.w = (float)a * sc; }
    Imcolor_t(ImU32 rgba)                                             { float sc = 1.0f/255.0f; Value.x = (float)((rgba>>IM_COL32_R_SHIFT)&0xFF) * sc; Value.y = (float)((rgba>>IM_COL32_G_SHIFT)&0xFF) * sc; Value.z = (float)((rgba>>IM_COL32_B_SHIFT)&0xFF) * sc; Value.w = (float)((rgba>>IM_COL32_A_SHIFT)&0xFF) * sc; }
    Imcolor_t(float r, float g, float b, float a = 1.0f)              { Value.x = r; Value.y = g; Value.z = b; Value.w = a; }
    Imcolor_t(const ImVec4& col)                                      { Value = col; }
    inline operator ImU32() const                                   { return ImGui::color_tConvertFloat4ToU32(Value); }
    inline operator ImVec4() const                                  { return Value; }

    // FIXME-OBSOLETE: May need to obsolete/cleanup those helpers.
    inline void    SetHSV(float h, float s, float v, float a = 1.0f){ ImGui::color_tConvertHSVtoRGB(h, s, v, Value.x, Value.y, Value.z); Value.w = a; }
    static Imcolor_t HSV(float h, float s, float v, float a = 1.0f)   { float r,g,b; ImGui::color_tConvertHSVtoRGB(h, s, v, r, g, b); return Imcolor_t(r,g,b,a); }
};
...
struct ImDrawList
{
    ...
    IMGUI_API void  AddRectFilledMulticolor_t(const ImVec2& p_min, const ImVec2& p_max, ImU32 col_upr_left, ImU32 col_upr_right, ImU32 col_bot_right, ImU32 col_bot_left);
    IMGUI_API void  AddRectFilledMultiReversed(const ImVec2& p_min, const ImVec2& p_max, ImU32 col_upr_left, ImU32 col_upr_right, ImU32 col_bot_right, ImU32 col_bot_left);
