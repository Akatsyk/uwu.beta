...
c_font::c_font(std::string font, uint32_t height, uint32_t weight, uint32_t highest_char, uint32_t flags) : font(
...
HRESULT c_font::init_device_objects(IDirect3DDevice9* dev)
{
	...
	const uint32_t height = -MulDiv(this->height, static_cast<uint32_t>(GetDeviceCaps(dc, LOGPIXELSY) * this->text_scale), 72);
	const auto font = CreateFontA(height, 0, 0, 0, this->weight, this->flags & D3DFONT_ITALIC,
		FALSE, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
		this->font == "astrix" ? DEFAULT_QUALITY : ANTIALIASED_QUALITY,
		DEFAULT_PITCH, this->font.c_str());
...
HRESULT c_font::draw_string(float x, float y, uint32_t color, const wchar_t* text, uint8_t flags)
{
	if (!this->dev)
		return E_FAIL;
...
HRESULT c_font::draw_radial(float x, float y, uint32_t color1, uint32_t color2, const wchar_t* text, uint8_t flags)
{
	if (!this->dev)
		return E_FAIL;
	...
	while (*text)
	{
		...
		if (c != ' ')
		{
			...
			const auto center = Vector2D((x - .5f) + w / 2.f, (y - .5f) + h / 2.f);
			const auto center_vertex = (Vector2D((x1 + x2) / 2.f, (y1 + y2) / 2.f));
			...
			clip.X = std::clamp(static_cast<uint32_t>(x), minX, maxX);
			clip.Y = std::clamp(static_cast<uint32_t>(y), minY, maxY);
