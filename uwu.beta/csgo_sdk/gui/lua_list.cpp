...
static int m_ScrollPosition;
static bool m_ScrollStateOnce;

LuaList::LuaList( std::string m_Name, int* m_Select, std::vector<std::string>& m_Items, int m_Num, int m_Addition )
{
	this->m_Name = m_Name;
...
LuaList::LuaList( std::string m_Name, int* m_Select, std::vector<std::string>& m_Items, std::string& m_Filter, int m_Num, int m_Addition )
{
	this->m_Name = m_Name;
	this->m_Select = m_Select;
	this->m_Items = &m_Items;
...
void LuaList::Scroll( int m_Limit )
{
	static size_t m_Lost;
	if ( m_Lost != m_Temp.size( ) ) {
		m_ScrollPosition = 0;
		m_Lost = m_Temp.size( );
	}
	...
	if ( !m_ScrollStateOnce && isHovered && m_Temp.size( ) > 16 )
	{
		...
		{
			m_ScrollPosition += Gui::m_Input.GetMouseWheel( ) * (-1);
			Gui::m_Input.SetMouseWheel( 0 );

			if ( m_ScrollPosition < 0 )
				m_ScrollPosition = 0;

			if ( m_ScrollPosition > ( int )m_prikol )
				m_ScrollPosition = ( int )m_prikol;
		}
	}
}
...
void LuaList::Draw( )
{
	...
	if ( !m_Temp.empty( ) )
	{
		for ( int i{ m_ScrollPosition }; (i < m_Temp.size( ) && DrawnItems < ItemsToDraw); i++ )
		{
			bool isHovered = Gui::m_Input.MousePointer
			(
