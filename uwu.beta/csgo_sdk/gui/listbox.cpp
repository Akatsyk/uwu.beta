...
ListBox::ListBox( std::string m_Name, int* m_Select, std::vector<std::string>& m_Items, int m_Num, int m_Addition )
{
	this->m_Name = m_Name;
...
ListBox::ListBox( std::string m_Name, int* m_Select, std::vector<std::string>& m_Items, std::string& m_Filter, int m_Num, int m_Addition )
{
	this->m_Name = m_Name;
	this->m_Select = m_Select;
	this->m_Items = &m_Items;
...
void ListBox::Scroll( int m_Limit )
{
	...
	if ( !m_ScrollState[ m_Num ] && isHovered && m_Temp.size( ) > 5 )
	{
		...
		float height_delta = some_shit + size_ratio - ( 103 );
		if ( height_delta > 0 )
			some_shit -= height_delta;

		float pos_ratio = float( some_shit ) / float( 103 );
...
void ListBox::Draw( )
{
	...
	if ( !m_Temp.empty( ) )
	{
		for ( int i{ m_ScrollPos[ m_Num ] }; ( i < m_Temp.size( ) && DrawnItems < ItemsToDraw ); i++ )
		{
			bool isHovered = Gui::m_Input.MousePointer
			(
				{ m_Pos.x + 6, ( m_Pos.y + 10 + DrawnItems * 20 ) },
				{ 200, 15 }
			);
			...
			g_Render->DrawString(m_Pos.x + 10, (m_Pos.y + 10 + DrawnItems * 20), m_Color, RenderImFonts::make_shadow, RenderImFonts::DefaultFont, m_Temp[i].first.c_str());

			DrawnItems++;
		}
	}

	g_Render->Rect(m_Pos.x + 3, m_Pos.y + 5, 265, 102, g_Options.GUI_CLRS.ListBoxLine_Clr, 2 );
	g_Render->Rect(m_Pos.x + 4, m_Pos.y + 6, 263, 100, g_Options.GUI_CLRS.ListBoxLine_2_Clr, 2 );
}
