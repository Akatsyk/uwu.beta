...
public:
    ...
    __forceinline Vector operator*(const Vector& vVec) const {
        auto& m = m_flMatVal;
        Vector vRet;
    ...
    __forceinline matrix3x4_t operator+(const matrix3x4_t& other) const {
        matrix3x4_t ret;
        auto& m = m_flMatVal;
        for (int i = 0; i < 12; i++) {
    ...
    __forceinline matrix3x4_t operator*(const float& other) const {
        matrix3x4_t ret;
        auto& m = m_flMatVal;
        for (int i = 0; i < 12; i++) {
    ...
    __forceinline matrix3x4_t operator*(const matrix3x4_t& vm) const {
