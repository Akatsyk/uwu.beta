...
bool C_BaseEntity::IsPlayer()
{
	...
	return client_class->m_ClassID == 40;
...
CCSWeaponInfo* C_BaseCombatWeapon::GetCSWeaponData()
{
	...
	return CallVFunction<Fn>(this, 461)(this);
...
bool C_BaseCombatWeapon::IsKnifeIDX()
{
	if (!this) //-V704
		return false;

	int idx = m_Item().m_iItemDefinitionIndex();
	...
	return idx == WEAPON_KNIFE || idx == WEAPON_BAYONET || idx == WEAPON_KNIFE_BUTTERFLY || idx == WEAPON_KNIFE_FALCHION
		...
		|| idx == WEAPON_KNIFE_URSUS || idx == WEAPON_KNIFE_WIDOWMAKER || idx == 503 || idx == 518
		|| idx == 517 || idx == 521 || idx == 525;
}

bool C_BaseCombatWeapon::IsRfileIDX()
{
	if (!this) //-V704
		return false;

	int weapon_id = m_Item().m_iItemDefinitionIndex();
...
bool C_BaseCombatWeapon::IsSmgIDX()
{
	if (!this) //-V704
		return false;
	...
	return weapon_id == WEAPON_MAC10 || weapon_id == WEAPON_MP7 || weapon_id == WEAPON_MP9 || weapon_id == WEAPON_P90 ||
		weapon_id == WEAPON_BIZON || weapon_id == WEAPON_UMP45 || weapon_id == WEAPON_MP5 || weapon_id == WEAPON_NEGEV || weapon_id == WEAPON_M249;
}

bool C_BaseCombatWeapon::IsShotgunIDX()
{
	if (!this) //-V704
		return false;
...
bool C_BaseCombatWeapon::IsPistolIDX()
{
	if (!this) //-V704
		return false;
...
bool C_BaseCombatWeapon::IsSniperIDX()
{
	if (!this) //-V704
		return false;
...
bool C_BaseCombatWeapon::IsGrenadeIDX()
{
	if (!this) //-V704
		return false;
...
{
	static auto eye_pos_fn = reinterpret_cast< float* (__thiscall*)(void*, Vector*) >(Utils::FindSig( "client.dll", "55 8B EC 56 8B 75 08 57 8B F9 56 8B 07 FF 90 ? ? ? ?" ));
	Vector eye_pos;
...
void C_BasePlayer::InvalidateBoneCache()
{
	...
	int iModelBoneCounter = **( int** )((( DWORD )(addr)) + 10);
...
CPaintKitStruct* C_EconItemView::GetPaintKit() {
	static auto m_Item = NetvarSys::Get().GetOffset( "DT_BaseAttributableItem", "m_Item" );
	static auto m_Data = Utils::FindInDataMap( GetPredMap(), "m_AttributeList");
	return *( CPaintKitStruct** )(( DWORD )this + m_Item + m_Data);
}
